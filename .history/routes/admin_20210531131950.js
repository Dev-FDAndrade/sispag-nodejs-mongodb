...
router.post('/updateCatPagamento', (req, res) => {
    var errors = [];
    if (!req.body.nome || typeof req.body.nome == undefined || req.body.nome == null) {
        errors.push({ error: "Campo nome da categortia obrigatório!" })
    }
    if (errors.length > 0) {
        //res.render('/admin/editCatPagamento/' + req.body.id, { errors: errors });
        req.flash('error_msg', 'Oops, não foi possivel atualizar a categoria! => ' + errors);
        res.redirect('/admin/editCatPagamento/6079a849ff5c42325c6b28a0', 401, { errors: errors });
    } else {
